...
def movel(robot_pose,velocity,blend_radius,fine_point, conf):
    cmd = movel_type()
    ...
    cmd.fine_point = fine_point
    cmd.extended = {
        "confdata": RR.VarValue(conf,"double[]")
    }
    return RR.VarValue(cmd,"experimental.robotics.motion_program.MoveLCommand")

def movej(robot_pose,velocity,blend_radius,fine_point,conf):
    cmd = movej_type()
    cmd.tcp_pose = robot_pose
    cmd.tcp_velocity = velocity
    cmd.blend_radius = blend_radius
...
def movec(robot_pose,robot_via_pose,velocity,blend_radius,fine_point, conf, conf_via):
    cmd = movec_type()
    cmd.tcp_pose = robot_pose
    ...
    cmd.extended = {
        "confdata": RR.VarValue(conf,"double[]"),
        "confdata_via": RR.VarValue(conf_via,"double[]")
    }
...
mp_cmds.append(movel(r1,0.5,0.02,False,(0,0,0,0)))
mp_cmds.append(movel(r2,0.5,0.2,True,(-1,-1,0,1)))

mp_cmds.append(movej(r1,0.5,0.02,False,(0,0,0,0)))
mp_cmds.append(movej(r3,0.5,0.2,True,( 0.,  1., -2.,  0.)))
